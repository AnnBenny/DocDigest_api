...
# --- Fake Embedder (Placeholder) ---
def embed(text: str) -> np.ndarray:
    return np.random.rand(384).astype("float32")  # Replace with real embeddings later
...
def generate_flashcards(file_path: str) -> list[tuple[str, str]]:
    text = extract_text_from_pdf(file_path)
    chunks = chunk_text(text)

    prompt = f"""You are a helpful tutor. Based on the following content, generate 5 flashcards in this format:
...
"""
    ...
    flashcards = []
    for part in output.split("Q:")[1:]:
        try:
            q, a = part.strip().split("A:")
